...
func (s *mcpServerImpl) RegisterNetworkTools() error {
	if !s.started {
		// Registration is possible even if the server is not running
	}

	// IP address retrieval tool
	ipAddressTool := mcp.NewTool("ip_address",
		mcp.WithDescription("Retrieves IP address information of the server"),
	)

	// Register tool handler
	s.server.AddTool(ipAddressTool, handleIPAddress)
...
func RegisterToolsToServer(mcpServer *server.MCPServer) error {
	...
	if err := RegisterNetworkToolsToServer(mcpServer); err != nil {
		return err
	}

	// Register SSH tools
	if err := RegisterSSHToolsToServer(mcpServer); err != nil {
		return err
	}
...
// RegisterNetworkToolsToServer registers network related tools to the MCP server
func RegisterNetworkToolsToServer(mcpServer *server.MCPServer) error {
	// IP address retrieval tool
	ipAddressTool := mcp.NewTool("ip_address",
		mcp.WithDescription("Retrieves IP address information of the server"),
	)
	...
	// Register tool handler
	mcpServer.AddTool(ipAddressTool, handleIPAddress)

	return nil
}

// RegisterSSHToolsToServer registers SSH related tools to the MCP server
func RegisterSSHToolsToServer(mcpServer *server.MCPServer) error {
	// SSH command execution tool
	sshCommandTool := mcp.NewTool("ssh_command",
		mcp.WithDescription("Execute commands on a remote host via SSH"),
		mcp.WithString("connection",
	...
	// Register SSH tool handlers
	mcpServer.AddTool(sshCommandTool, handleSSHCommand)
	mcpServer.AddTool(sshConnectionListTool, handleSSHConnectionList)
	mcpServer.AddTool(sshConnectionRegisterTool, handleSSHConnectionRegister)
